...
| `An` 41 00 `PT` | B0 36 `GR` `PT` | `An` 42 00 `PT` | B0 41 00 71 | 통화 버튼 누름, 월패드가 통화 거절 |
| `An` 41 00 `PT` | B0 36 `GR` `PT` | `An` 36 `GR` `PT` | B0 42 00 72 | 통화 버튼 누름, 통화 시작 |
| `An` 41 00 `PT` | B0 42 00 72 | - | - | 통화 중 |
| `An` 41 00 `PT` | B0 3B 00 `PT` | `An` 3B 00 `PT` | B0 42 00 72 | 통화중 문열림 버튼 누름, 승인됨 |
| `An` 42 00 `PT` | B0 41 00 71 | - | - | 다른 인터폰이 통화 중 (거절 패킷과 같음) |
| `An` 31 00 `PT` | B0 31 00 01 | - | - | 집 현관 초인종 눌림 |
| `An` 32 00 `PT` | B0 32 00 02 | - | - | 공동현관 초인종 눌림 |
| `An` 3E `GR` `PT` | B0 3E `GR` `PT` | - | - | 초인종 꺼짐. |

* `GR`: 01: 현관, 06: 공동현관
* 아직 검증 중이라 오류가 있을 수 있습니다.
...
| AB 41 00 `PT` | B0 41 `XX` `PT` | 가스밸브 열림/닫힘 상태 확인. XX (00: 열림, 01: 닫힘) |
...
* `BM`: 조명 상태 비트맵. 해당하는 방의 조명 켜짐/꺼짐 상태를 각각 1bit으로 나타냅니다.
    * 예시
        ...
        * 윈도우10 기본 계산기의 프로그래머를 사용하시면 편리합니다.
...
* `XX`: 00: 열림, 01: 닫힘
* AD 41에 B0 56으로 응답하면 AD 41 - AD 56 - AD 52 의 반복이고, B041로 응답하면 AD 41 - AD 52 의 반복입니다.
...
* `XX`: 설정온도 (16진수)
* `YY`: 현재온도 (16진수)
* 장치 응답 중 0xFF 가 고정으로 있습니다. 0x80보다 큰 값이면서 패킷 시작이 아닌 유일한(?) 경우입니다.
...
* `ZZ`: 현재 소모량 (10진수 ZZZZ.ZZ)
    * 예를 들어 B0 6F 00 12 34 56 2F라면, 현재 전력소모량은 1234.56 Wh 입니다.
* 단위는 전기는 Wh, 가스/수도는 m³ 입니다.
* 사례가 적어서 교차검증이 어려운데, 월패드 실시간에너지 페이지에서 검증이 가능하니 차이가 있다면 제보 부탁 드립니다.
...
* 양상을 봤을 때 B0 01 05 00 34 에 뭔가 정보가 들어있고 CE 41 01 00 0E 는 ACK인 것 같습니다. 뭔지 모르는 정보를 월패드에 계속 던지기가 불안해서 더 이상 시도는 안해봤습니다.
